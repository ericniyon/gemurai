...
import { prismaSimple, connectDatabase } from "@/lib/prisma-simple"
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Test simple query
    try {
      const result = await prismaSimple.$queryRaw`SELECT 1 as test, NOW() as current_time`
    ...
    console.error("❌ Unexpected error in simple-db test:", error)
    return NextResponse.json({
      success: false,
